...
/**
 * @brief Cria um novo cliente, pede a informação do cliente ao utilizador e insere-o no primeiro elo da lista
 * 
 * @param listaClientes apontador para o apontador da lista clientes
 */
...
/**
 * @brief It prints the information of all the vehicles that are available
 * 
 * @param listaVeiculos pointer to the first element of the list
 * 
    ...
    while (fgets(linha, sizeof(linha), arquivo) != NULL) {
        ...
        if (*listaVertices == NULL) {
            *listaVertices = novoVertice;
            ...
            // Percorre a lista de vértices até o último elemento
...
void adicionarVertice(vertice** listaVertices, char* nomeVertice) {
    // Verifica se o vértice já existe na lista
    vertice* temp = *listaVertices;
    while (temp != NULL) {
...
int existeVertice(vertice* listaVertices, char verticeNome[]) {
    ...
    while (temp != NULL) {
        if (strcmp(temp->verticeOrigem, verticeNome) == 0) {
            // O vértice foi encontrado na lista
            return 1;
        }
        temp = temp->next;
    }
    // O vértice não foi encontrado na lista
    return 0;
}
...
void criarAresta(vertice** listaVertices, vertice** listaArestas, char verticeOrigem[], char verticeDestino[], float distancia) {
    ...
    if (origem != NULL && destino != NULL) {
        ...
        if (temp == NULL) {
            // O vértice de origem não está na lista de arestas, cria um novo nó
            ...
            *listaArestas = novoVertice;
            ...
            // O vértice de origem já está na lista de arestas, adiciona a nova aresta ao nó existente
...
void inserirVeiculoEmVertice(vertice* listaVertices, char verticeNome[], int codigoVeiculo, veiculo* listaVeiculos) {
    ...
    while (temp != NULL) {
        if (strcmp(temp->verticeOrigem, verticeNome) == 0) {
            // Encontra o veículo com o código correspondente
